...
#    elif defined(USE_HAL_DRIVER)
        HAL_TIM_PWM_Start(&IrsndTimer, IRSND_TIMER_CHANNEL);
        HAL_TIM_Base_Start(&IrsndTimer);
#    endif
...
#    elif defined(USE_HAL_DRIVER)
        HAL_TIM_Base_Stop(&IrsndTimer);
        HAL_TIM_PWM_Stop(&IrsndTimer, IRSND_TIMER_CHANNEL);
#    endif
...
#    elif defined(USE_HAL_DRIVER)
       static IRSND_FREQ_TYPE   last_freq = 0;
       uint32_t                 TimeBaseFreq;

        /* Get and store timer clock in variable */
#      if ((IRSND_TIMER_NUMBER >= 2) && (IRSND_TIMER_NUMBER <= 5)) || ((IRSND_TIMER_NUMBER >= 12) && (IRSND_TIMER_NUMBER <= 14))
        TimeBaseFreq = HAL_RCC_GetPCLK1Freq();
#      else
...
#      endif
        ...
        if (freq != last_freq)
        {
           ...
           IrsndTimer.Init.Period     = freq - 1;
           if (HAL_TIM_PWM_Init(&IrsndTimer) != HAL_OK)
           {
             /* Initialization Error */
//             Error_Handler();
           }

           /* PWMx Mode configuration */
           TIM_OC_InitStructure.Pulse = (freq + 1) / 2;
           if (HAL_TIM_PWM_ConfigChannel(&IrsndTimer, &TIM_OC_InitStructure, IRSND_TIMER_CHANNEL) != HAL_OK)
           {
             /* Configuration Error */
//             Error_Handler();
           }
...
#    elif defined(USE_HAL_DRIVER)
        /* GPIOx clock enable */
        IRSND_PORT_CLK_ENABLE();

        /* GPIO Configuration */
        GPIO_InitStructure.Pin       = IRSND_BIT;
        GPIO_InitStructure.Mode      = GPIO_MODE_AF_PP;
        GPIO_InitStructure.Speed     = GPIO_SPEED_LOW;
        GPIO_InitStructure.Pull      = GPIO_NOPULL;
#      if defined(STM32L151xB)
        GPIO_InitStructure.Alternate = IRSND_PORT_ALTERNATE_FUNCTION;
#      endif
        HAL_GPIO_Init(IRSND_PORT, &GPIO_InitStructure);

        /* TIMx clock enable */
        IRSND_TIMER_CLK_ENABLE();

        /* Time base configuration */
        IrsndTimer.Instance           = IRSND_TIMER;
        IrsndTimer.Init.Period        = -1; // dummy value (don't set to 0), will be initialized later;
        IrsndTimer.Init.Prescaler     = IRSND_TIMER_PRESCALER - 1;
        IrsndTimer.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
        IrsndTimer.Init.CounterMode   = TIM_COUNTERMODE_UP;
        // HAL_TIM_PWM_Init() is called inside irsnd_set_freq()

        /* PWMx Mode configuration */
        TIM_OC_InitStructure.Pulse       = 0; // dummy value, will be initialized later
        TIM_OC_InitStructure.OCMode      = TIM_OCMODE_PWM1;
        TIM_OC_InitStructure.OCPolarity  = TIM_OCPOLARITY_HIGH;
        TIM_OC_InitStructure.OCFastMode  = TIM_OCFAST_DISABLE;
        TIM_OC_InitStructure.OCIdleState = TIM_OCIDLESTATE_RESET;
        // HAL_TIM_PWM_ConfigChannel() is called inside irsnd_set_freq()
#    endif
