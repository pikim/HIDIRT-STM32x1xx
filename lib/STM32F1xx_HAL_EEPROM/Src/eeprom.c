/**
  ...
  * @version V3.1.0
  * @date    07/27/2009
  * @brief   Provides all the EEPROM emulation firmware functions according to
  *          ST application note AN2594 "EEPROM emulation in STM32F10x
  *          microcontrollers". This library is based on the SPL version
  *          delivered with the above app note, but was modified to work with
  *          the HAL from STM32Cube.
  ******************************************************************************
  * @copy
  *
...
/**
  ...
  */
static inline HAL_StatusTypeDef EE_ErasePage(uint32_t PageAddress)
{
  HAL_StatusTypeDef FlashStatus;

  /* Wait for last operation to be completed */
  FlashStatus = FLASH_WaitForLastOperation(EEPROM_ERASE_TIMEOUT);
  ...
  if (FlashStatus == HAL_OK)
  {
    ...
    /* Disable the PER Bit */
    /* Seems to be necessary after page erase. Is not documented, but taken
     * from the SPL. */
  ...
  switch (PageStatus0)
  {
    ...
    case RECEIVE_DATA:
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        for (VarIdx = 0; VarIdx < NUMBER_OF_VARIABLES; VarIdx++)
        {
...
HAL_StatusTypeDef EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data)
{
  ...
  uint16_t AddressValue, ValidPage;
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  uint16_t ValidPage, VarIdx, DataVar;
  uint16_t EepromStatus, ReadStatus;
  ...
  FlashStatus = EE_ErasePage(OldPageAddress);
  /* If erase operation was failed, a Flash error code is returned */
  if (FlashStatus != HAL_OK)
  {
    return FlashStatus;
